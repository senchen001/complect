...
require_once('irbis_record_class.inc');

/** основной класс подключения к ирбису */
class irbis64
{
    ...
    function __construct($ip = '127.0.0.1', $port = 6666, $login = '1', $pass = '1', $db = 'IBIS')
    {
        $this->set_db($db);
        $this->set_server($ip, $port);
        $this->set_user($login, $pass);
        $this->set_arm('C');       // C - Каталогизатор
        $this->set_id('388623');   // Идентификатор сессии
    }
    function __destruct()
    {
        $this->logout();
    ...
    /** Расформатировать запись
     * @param int $mfn - мфн записи
     * @param string $format - формат ирбис
     */
    function record_format($mfn, $format)
    {
        $packet = implode("\n", array('G', $this->arm, 'G', $this->id, $this->seq, '', '', '', '', '', $this->db, $format, 1, $mfn));
        $packet = strlen($packet) . "\n" . $packet;
	...
	*/
        ...
        for ($i = 13; $i < $c; $i++) {
            ...
            $field_val = $matches[2];
            $record['fields'][$field_num][count(($record['fields'][$field_num]) ?? []) + 1] = $this->parse_field($field_val);
        }
        return new irbisRecord($record);
    }
...
//dd($answer);
        ...
        $c = count($answer) - 1;
        for ($i = 12; $i < $c; $i++) {
            $ret['records'][] = explode('#', $answer[$i]);
        }
        
        return $ret;
    }

    private function send($packet, $debug = false)
    {
        if ($this->sock === false) return false;
    ...
    private function parse_field(&$field)
    {
        ...
        if (str_contains($field, '^')) {
            ...
            foreach ($matches as $match) {
                $ret[mb_strtoupper((string)substr($match, 0, 1))] = substr($match, 1);
            }
        }
        return $ret;
    }
